---
title: Vietnam eVisa Shows "Unfulfilled"? What It Means and How to Fix It
description: Portal status stuck on Unfulfilled? Usually payment or a missing step—not a rejection. Fix payment, complete uploads, or contact support before you duplicate your application.
slug: evisa-unfulfilled-status
date: 2026-06-23
updated: 2026-06-23
index: true
priority: 0.95
howToTotalTime: PT2H
faq:
  - question: What does "Unfulfilled" mean on a Vietnam eVisa application?
    answer: Unfulfilled usually means a required step was not completed—most often failed or pending payment, an unfinished upload, or a session that expired before checkout finished. It is not the same as "Rejected."
  - question: Is Unfulfilled the same as rejected?
    answer: No. Rejected means immigration declined your application. Unfulfilled means the filing did not fully complete in the system—payment capture, document upload, or final submission may be missing.
  - question: Should I apply again if my Vietnam eVisa says Unfulfilled?
    answer: Not immediately. Check your bank for charges, search all email folders for confirmation, and contact support with your reference number. Duplicate paid applications can create conflicting records.
  - question: Can Unfulfilled status change to approved on its own?
    answer: Only if the missing step completes—such as a delayed payment posting or a document upload finishing. If payment never succeeded, the application will not move to processing without action.
howToSteps:
  - name: Check whether payment actually posted
    text: Log into your bank or card app and look for a completed charge to an immigration or eVisa merchant—not just a temporary hold. No completed charge usually means the application never entered the queue.
  - name: Search every email folder for confirmation
    text: Look for a reference number in inbox, spam, and promotions. An Unfulfilled portal with a valid payment receipt may be display lag—save screenshots for support.
  - name: Confirm uploads and final submit completed
    text: Re-open the portal if possible and verify passport scan and portrait photo uploaded. A closed browser mid-upload can leave status Unfulfilled without an error message.
  - name: Contact support before paying again
    text: Send your reference number, payment proof or decline screenshot, and travel date. Ask whether to retry payment, complete uploads, or start a fresh filing.
  - name: Retry payment in a clean session if advised
    text: Disable VPN, match billing to your bank records, and attempt one controlled payment—see our payment failed guide if the card declines again.
---
...
| **Processing / Pending** | Paid and in review queue | Wait within your tier window—see [stuck processing](/troubleshooting/stuck-processing) |
| **Approved** | Decision issued—check email for PDF | Download and verify all fields |
| **Rejected** | Decision issued—denial | Read email for reason—see [rejection guide](/troubleshooting/evisa-rejected-reapply) |
...
You may see Unfulfilled in the portal while your bank shows only a **pending hold**, not a posted charge. Read [payment failed fixes](/troubleshooting/payment-failed) before retrying.
...
**Avoid paying twice.** Duplicate filings are a top cause of extended review—see [duplicate application guide](/troubleshooting/duplicate-evisa-application).
...
When re-filing, copy every field from your passport carefully—especially [date of birth format](/troubleshooting/wrong-date-of-birth-evisa) and [name spelling](/troubleshooting/wrong-name-evisa).
...
1. Save payment receipt and reference number
2. Contact support immediately with proof of charge
3. Do **not** pay again until support confirms the first payment is lost
4. Escalate if departure is within 72 hours—[Super Urgent tier](/apply) may apply after payment alignment
...
| **Processing** | **Yes** | **Yes** | Panic at 48 hours—see [48-hour guide](/troubleshooting/evisa-processing-48-hours) |
...
1. Payment never completed → Unfulfilled
2. Payment completed but confirmation went to **wrong email** → you think nothing happened
...
1. Upload passport scan → wait until thumbnail appears
2. Upload portrait → wait until preview shows
3. Review every data field against passport MRZ
4. Pay once on stable connection **without VPN**
5. Wait for **confirmation page** plus **email with reference number**
6. Screenshot both before closing the tab
...
- `vietnam evisa unfulfilled meaning`
- `vietnam evisa payment taken but unfulfilled`
- `evisa vietnam status unfulfilled not rejected`
- `vietnam visa application incomplete payment`
...
If Unfulfilled follows a debit decline, call your bank to approve Vietnam/immigration merchants, then retry **once** on credit card if available—see full [payment failed](/troubleshooting/payment-failed) workflow.
...
- [Payment failed at checkout](/troubleshooting/payment-failed)
- [eVisa pending 5+ days](/troubleshooting/stuck-processing)
- [48 hours still processing](/troubleshooting/evisa-processing-48-hours)
- [Weekend and holiday delays](/troubleshooting/weekend-processing)
- [Duplicate application mistakes](/troubleshooting/duplicate-evisa-application)

## Departure soon?

Unfulfilled status with an upcoming flight needs same-day action—not waiting for the portal to update itself. [Contact support](/contact) with payment proof or [start a rush application](/apply) after support confirms your next step.
